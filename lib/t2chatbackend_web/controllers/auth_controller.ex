defmodule T2chatbackendWeb.AuthController do
  ...
  def register(conn, params) do
    with {:ok, %{"user" => supa_user}} <-
           Supabase.register_user(params),
         {:ok, user} <- Accounts.find_or_create(%{
    ...
    def login(conn, %{"email" => email, "password" => password} = params) do
      ...
      with {:ok, %{"access_token" => access_token, "refresh_token" => refresh_token, "user" => supa_user}} <-
    ...
    def refresh(conn, %{"refresh_token" => refresh_token}) do
      case Supabase.refresh_token(refresh_token) do
        {:ok, %{"access_token" => access_token, "refresh_token" => new_refresh_token, "user" => supa_user}} ->
